...
/**
 * Клас <code>Printer</code> моделює роботу простого принтера.
 * Принтер складається з трьох компонентів:
 * <ul>
 *     <li>картриджа ({@link Cartridge})</li>
 *     <li>лотка для паперу ({@link PaperTray})</li>
 *     <li>модуля підключення ({@link Connection})</li>
 * </ul>
 * Клас підтримує друк документів, перевірку готовності, додавання паперу,
 * заміну картриджа та ведення лог-файлу.
 */
public class Printer {
    ...
    private PrintWriter fout; // лог-файл
    ...
    public Printer() throws FileNotFoundException {
        ...
        cartridge = new Cartridge("Black", 100); // чорнила на 100 сторінок
        paperTray = new PaperTray(10);           // 10 аркушів у лотку
        connection = new Connection("USB");
        poweredOn = false;
        printedPages = 0;
    ...
    public Printer(String type, int inkPages, int initialPaper, String connectionType)
        ...
        cartridge = new Cartridge(type, inkPages);
        paperTray = new PaperTray(initialPaper);
        connection = new Connection(connectionType);
        poweredOn = false;
        printedPages = 0;
     ...
     */
    public void replaceCartridge(Cartridge newCartridge) {
